...
const Onboarding = () => {
  ...
  return (
    ...
    >
      {/* NOTE: TouchableOpacity is the mobile version of a button */}
      <TouchableOpacity
        onPress={() => {
          router.replace("/(auth)/sign-up");
        }}
        // sets the button to the far right location
        className={"w-full flex justify-end items-end p-5"}
      >
        <Text className={"text-black text-md font-JakartaBold"}>Skip</Text>
      </TouchableOpacity>
      {/* NOTE: Swiper allows the user to swipe to the next card/screen */}
      <Swiper
        ref={swiperRef}
        loop={false}
        dot={
